...
def tif2area(intif,inshp,class_dic,outjson
             ,tempfp=os.path.join(os.path.dirname(__file__), 'temp_tif2json')
             ,isdel = 1,xres = 10,yres = 10,year=time.strftime('%Y')
             ):
    '''
    intif:待统计的tif数据
    inshp:待统计的shp数据
    class_dic:tif数据栅格值对应的属性名 字典类型
    outjson:统计后的数据文件,输出的面积单位默认为亩
    tempfp:中间数据文件临时存储位置
    isdel:是否删除临时文件
    xres:lon分辨率，默认为10m
    yres:lat分辨率，默认为10m
    year:数据年份，默认为数据生产是的年份，如果生产的历史数据记得修改
    '''
    ...
    if not os.path.exists(tempfp):
        ...
        os.makedirs(tempfp)
    # 投影栅格和矢量数据
    outtif,outshp = projtif(intif,inshp,tempfp,xres = xres,yres = yres)
    # 计算面积导出json
    calculate_tif_area(outtif,outshp,class_dic,outjson,year = year)
    # 判断是否删除临时文件
    if isdel==1:
